...
then
	...
	kubectl -n $namespace scale deployment rugby-app-test-loadtest-jmeter-slaves --replicas=0

	# Refaz setando o numero de replicas necessario (smoke test usa apenas 1):
	kubectl -n $namespace scale deployment rugby-app-test-loadtest-jmeter-slaves --replicas=$1
